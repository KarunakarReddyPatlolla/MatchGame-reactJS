...
class MatchGame extends Component {
  ...
  onClickImage = thumbnailUrl => {
    const {displayImageUrl} = this.state
    const clickedImage = imagesList.filter(
      eachObject => eachObject.thumbnailUrl === thumbnailUrl,
    )

    if (displayImageUrl === clickedImage[0].imageUrl) {
      this.setState(prevState => ({
        score: prevState.score + 1,
      }))
      const randomUrl =
        imagesList[Math.floor(Math.random() * (imagesList.length - 1))].imageUrl
      this.setState({displayImageUrl: randomUrl})
    } else {
      clearInterval(this.intervalId)
      this.setState({isTimerRunning: false})
    }
  ...
  renderPlayGame = () => {
    const {activeTab, displayImageUrl} = this.state
    const finalImagesList = this.getActiveTabImages(activeTab)

    return (
      <>
        <ul className="image-container">
          <li>
            <img src={displayImageUrl} alt="match" className="display-image" />
          </li>
        </ul>
        ...
        <ul className="images-display-container">
          {finalImagesList.map(eachImage => (
            <ImageItem
  ...
  render() {
    const {score, timeInSeconds, isTimerRunning} = this.state
          ...
          <div className="score-container">
            ...
            <li className="score-container">
              <img
                ...
                className="icon"
              />
              <p className="span-text">{timeInSeconds} Sec</p>
            </li>
          </div>
        </nav>
        <div className="main-container">
          {isTimerRunning ? this.renderPlayGame() : this.renderScoreCard()}
        </div>
      </div>
    )
  }
